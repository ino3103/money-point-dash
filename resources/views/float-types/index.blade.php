...
@section('title', config('app.name') . ' - ' . __('auth.float_types'))

@section('content')
    <div class="mx-auto max-w-7xl p-4 pb-20 md:p-6 md:pb-6" x-data="floatTypesDataTable()" x-init="loadData()">
        <div class="grid grid-cols-12 gap-4 md:gap-6">
            <!-- Stats Cards -->
            <div class="col-span-12 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4">
                <!-- Total Float Types Card -->
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('auth.total_float_types') }}</p>
                ...
                <!-- Active Float Types Card -->
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('auth.active_float_types') }}</p>
                            <p class="mt-2 text-2xl font-semibold text-gray-800 dark:text-white/90"
                                x-text="data.filter(f => f.is_active).length">0</p>
                        </div>
                ...
                <!-- Inactive Float Typees Card -->
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('auth.inactive_float_types') }}</p>
                            <p class="mt-2 text-2xl font-semibold text-gray-800 dark:text-white/90"
                                x-text="data.filter(f => !f.is_active).length">0</p>
                        </div>
                ...
                <!-- Float Types with Code Card -->
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('auth.with_code') }}</p>
                            <p class="mt-2 text-2xl font-semibold text-gray-800 dark:text-white/90"
                                x-text="data.filter(f => f.code && f.code !== '-').length">0</p>
                        </div>
                    ...
                    <div class="px-5 py-4 sm:px-6 sm:py-5">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('auth.manage_float_types') }}
                                </p>
                            </div>
                            <a href="{{ route('float-types.create') }}"
                        ...
                        <div
                                ...
                                <div class="flex items-center justify-center">
                                    <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                        <select x-model="statusFilter"
                                            class="dark:bg-dark-900 h-9 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none py-2 pl-3 pr-8 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
                                            @change="loadData()">
                                            <option value="">{{ __('auth.all_status') }}</option>
                                            <option value="active">{{ __('auth.active_only') }}</option>
                                            <option value="inactive">{{ __('auth.inactive_only') }}</option>
                                        </select>
                                        <span
                                            class="absolute right-2 top-1/2 z-30 -translate-y-1/2 text-gray-500 dark:text-gray-400">
                                            <svg class="stroke-current" width="16" height="16"
                            ...
                            <div class="max-w-full overflow-x-auto">
                                <div class="min-w-[1000px]">
                                            ...
                                            <div class="col-span-12 text-center">
                                                <div class="flex flex-col items-center justify-center">
                                                    ...
                                                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                                        ...
                                                        <span x-show="!search && !statusFilter">
                                                            {{ __('auth.create_first_float_type') }}
                                                        </span>
                                                    </p>
                                    ...
                                    <template x-for="(floatType, index) in paginatedData" :key="floatType.id">
                                        <div class="grid grid-cols-12 border-t border-gray-100 dark:border-gray-800">
                                            ...
                                            <div
                                                ...
                                                <span
                                                    ...
                                                    :title="floatType.description || '-'"
                                                    x-text="floatType.description || '-'">
                                                </span>
                                            </div>
                                            <div
                                                class="col-span-1 flex items-center border-r border-gray-100 px-4 py-[17.5px] dark:border-gray-800">
                                                <div x-html="floatType.status"></div>
                                            </div>
                                            <div
                                                class="col-span-2 flex items-center border-r border-gray-100 px-4 py-[17.5px] dark:border-gray-800">
                                                <span class="text-theme-sm text-gray-700 dark:text-gray-400"
                                                    x-text="floatType.created_at || '-'">
                                                </span>
                                            </div>
                                            <div class="col-span-2 flex items-center justify-end px-4 py-[17.5px]">
                                                <div class="flex items-center gap-2" x-html="floatType.actions">
                                                </div>
                                            </div>
                                        </div>
                                    </template>
    ...
    @push('scripts')
        ...
        <script>
            function editFloatType(floatTypeId) {
                window.location.href = `/float-types/${floatTypeId}/edit`;
            }

            function deleteFloatType(floatTypeId, floatTypeName) {
                Swal.fire({
                    title: '{{ __('auth.are_you_sure') }}?',
                    text: `{{ __('auth.do_you_want_to_delete_float_type') }} "${floatTypeName}"? {{ __('auth.this_action_cannot_be_undone') }}!`,
                    icon: 'warning',
                    ...
                    preConfirm: async () => {
                        try {
                            const response = await fetch(`/float-types/${floatTypeId}`, {
                                method: 'DELETE',
                                headers: {
                            ...
                            if (!response.ok) {
                                const errorData = await response.json().catch(() => ({
                                    message: '{{ __('auth.failed_to_delete_float_type') }}'
                                }));
                                throw new Error(errorData.message ||
                                    '{{ __('auth.failed_to_delete_float_type') }}');
                            }
                    ...
                    if (result.isConfirmed) {
                        Swal.fire({
                            ...
                            text: `{{ __('auth.float_type_deleted_success') }} "${floatTypeName}".`,
                            icon: 'success',
                            timer: 2000,
                            showConfirmButton: false
                        }).then(() => {
                            const table = document.querySelector('[x-data*="float-typesDataTable"]');
            ...
            function floatTypesDataTable() {
                return {
                    ...
                    init() {
                        console.log('Alpine.js float-typesDataTable initialized');
                    },
                    ...
                    async loadData() {
                        ...
                        try {
                            ...
                            const url = `{{ route('float-types.data') }}?${params.toString()}`;
                            ...
                            if (result.data && Array.isArray(result.data)) {
                                this.data = result.data.map((item, index) => {
                                    return {
                                        id: item.id,
                                        DT_RowIndex: item.DT_RowIndex || ((this.currentPage - 1) * this
                                            .perPage +
                                            index + 1),
                                        name: item.name,
                                        code: item.code,
